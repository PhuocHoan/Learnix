...
1. **Or start only PostgreSQL** (if you want to run backend/frontend locally):
...
make pre       # Run pre-commit checks (format, lint, typecheck, test, build)
```
...
```bash
pnpm precommit    # Run all checks: format → lint → typecheck → test → build
pnpm format       # Format all files with Prettier
pnpm lint         # Run ESLint on frontend and backend
pnpm typecheck    # TypeScript type checking
pnpm test         # Run all tests (Vitest for frontend, Jest for backend)
