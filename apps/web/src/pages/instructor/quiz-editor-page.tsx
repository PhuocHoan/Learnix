            ...
            setOrderedQuestions((items) => {
                ...
                if (quiz) {
                    ...
                    void quizzesApi.reorderQuestions(quiz.id, ids).catch(() => {
                        ...
                        void queryClient.invalidateQueries({ queryKey: ['quiz', 'lesson', lessonId] });
                    });
                }
                ...
                <div className="space-y-4">
                    ...
                    {quiz?.questions?.length === 0 && !editingQuestionId && (
                        <div className="text-center py-12 border-2 border-dashed rounded-lg bg-muted/50">
                            ...
                            <Button variant="outline" onClick={() => setEditingQuestionId('new')}>
                        ...
                        {question.options.map((opt, i) => {
                            ...
                            return (
                                <div key={i} className={cn(
                                    ...
                                    isSelectedCorrect ? "border-green-500 bg-green-50 text-green-700" : "border-border bg-background"
                                )}>
...
function QuestionEditor({ quizId, question, onCancel, onSave }: { quizId: string, question?: Question, onCancel: () => void, onSave: () => void }) {
    ...
    const mutation = useMutation({
        mutationFn: (data: QuestionFormData) => {
            ...
            if (!question) { return Promise.reject(new Error('Question is missing')); }
            ...
            <CardContent className="p-6">
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    <div className="space-y-2">
                        <div className="flex justify-between items-center">
                            <label className="text-sm font-medium" htmlFor="question-text">Question Text</label>
                            <div className="flex items-center gap-2">
                                <label className="text-sm font-medium text-muted-foreground mr-2" htmlFor="points-input">Points:</label>
                                <Input
                                    id="points-input"
                                    type="number"
                        ...
                        {showImageUpload && (
                            <div className="p-4 border border-dashed rounded-lg bg-muted/20 mb-2">
                                <p className="text-xs text-muted-foreground mb-2">Upload an image to insert into the question.</p>
                                <ImageUpload
                                    onChange={(url) => {
                                        if (url) {
                                            const currentText = form.getValues('questionText');
                                            form.setValue('questionText', `${currentText}\n\n![Image](${url})`);
