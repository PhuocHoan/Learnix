...
export function QuizPlayer({ quiz, onComplete }: QuizPlayerProps) {
    ...
    if (submission && !submission.completedAt && !responsesLoaded && !isLoadingSubmission && !isRetaking) {
        setAnswers(submission.responses || {});
        setResponsesLoaded(true);
    }

    const saveProgressMutation = useMutation({
        mutationFn: (data: Record<string, string>) => quizzesApi.saveProgress(quiz.id, data),
        onSuccess: () => {
    ...
    const submitMutation = useMutation({
        mutationFn: (data: Record<string, string>) => quizzesApi.submitQuiz(quiz.id, data),
        onSuccess: () => {
            toast.success('Quiz submitted successfully!');
            void queryClient.invalidateQueries({ queryKey: ['quiz-submission', quiz.id] });
            setIsRetaking(false);
            if (onComplete) {
                onComplete();
            }
        ...
        <Card className="w-full max-w-3xl mx-auto">
            ...
            <CardFooter className="flex justify-between">
                ...
                <Button onClick={handleNext} disabled={!answers[currentQuestion.id]}>
                    {currentQuestionIndex === questions.length - 1 && submitMutation.isPending && 'Submitting...'}
                    {currentQuestionIndex === questions.length - 1 && !submitMutation.isPending && 'Submit Quiz'}
...
function QuizResultView({ quiz, submission, onRetake }: { quiz: Quiz, submission: QuizSubmission, onRetake: () => void }) {
    const isPass = submission.percentage >= 70; // Hardcoded pass threshold for now

    return (
        <div className="space-y-8 w-full max-w-3xl mx-auto">
            <Card className={isPass ? 'border-green-500/20 bg-green-50/10' : 'border-red-500/20 bg-red-50/10'}>
                            ...
                            <CardContent className="pt-2 text-sm space-y-2">
                                <div className="grid gap-2">
                                    {question.options.map((opt, i) => {
                                        ...
                                        const isTheCorrectAnswer = question.correctAnswer === letter;

                                        let className = "p-3 rounded border flex justify-between items-center";
                                        ...
                                        return (
                                            <div key={i} className={className}>
                                                ...
                                                {isTheCorrectAnswer && <Check className="w-4 h-4 text-green-600" />}
